...
begin
  if LogOpen then
  begFileClose(FLogFileHandle); { *Převedeno z CloseHandle* }Handle);
...
begin
  ...
  DetailsBtn.Visible := FileExists(ChangeFileExt(Application.ExeName, '.MAP'));
  if DetailsBtn.Visible then
    Tag := 1
  else
    Tag := 0;
end;
...
procedure TExceptionDialog.bTerminateClick(Sender: TObject);
begin
